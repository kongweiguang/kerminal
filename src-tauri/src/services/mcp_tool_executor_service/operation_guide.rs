        ...
        "sftp" => guide_plan(
            ...
            vec![
                ...
                guide_step(
                    ...
                    "Queue one copy with canonical source and destination endpoints. This returns a transfer id immediately; it does not wait for connection or file bytes. This covers local-to-remote, remote-to-local, same-host, and cross-host remote copies for files or directories.",
                    &["source", "destination", "kind", "conflictPolicy"],
                    "conflictPolicy is required and must be overwrite, skip, or rename. idleTimeoutSeconds is optional, must be 30-3600, and limits only consecutive inactivity; copying never deletes the source, and the MCP host owns approval.",
                ),
                guide_step(
                    "track",
