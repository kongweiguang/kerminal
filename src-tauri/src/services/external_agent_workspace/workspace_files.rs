...
impl ExternalAgentWorkspaceService {
    ...
    /// 生成 Agent 会话规则，将 global scope、targetBinding 首选目标和可恢复的终端操作写入受管工作区。
    ///
...
- Every external Agent session uses global terminal scope across all Kerminal tabs. Prefer the current `targetBinding` as the default terminal; it is a convenience target, not an access restriction, so other user terminals remain operable.
...
- For SFTP copies, call `sftp.transfer.enqueue` only to receive a transfer id, track it through `sftp.transfer.list`, and use `sftp.transfer.cancel` only to stop an explicitly requested task. The 60-second MCP timeout protects those short calls only, never the queued transfer. `idleTimeoutSeconds` protects consecutive missing byte progress rather than total duration; after `failureKind = "idleTimeout"`, re-enqueue the same endpoints and adopted timeout to resume retained partial data. Do not call retired synchronous `sftp.upload*` or `sftp.download*` tools.
...
- Every external Agent session uses global terminal scope across all Kerminal tabs. Prefer `targetBinding` from the session context as the current terminal; it marks a preferred target only and does not restrict other user terminals.
...
- For SFTP file copies, use `sftp.transfer.enqueue` only to obtain a transfer id, then `sftp.transfer.list` to track it and `sftp.transfer.cancel` only for an explicit stop. The 60-second MCP call timeout covers these short calls, not the background transfer. `idleTimeoutSeconds` protects only consecutive missing byte progress; a long transfer with progress has no total-duration limit. On `failureKind = "idleTimeout"`, re-enqueue the same endpoints and returned timeout to resume retained partial data. Do not call retired synchronous `sftp.upload*` or `sftp.download*` tools.
...
- Every external Agent session uses global scope across all Kerminal tabs. `targetBinding` identifies the current preferred terminal only; it is not an access restriction, and the right-panel Agent TUI is never a user-terminal member.
- For ordinary commands, inspect the current `targetBinding` with `terminal.snapshot`, then use `terminal.write` through the visible PTY so input and output remain visible in the user's left PTY. Use `kerminal.agent.target_context` and `terminal.list` only when the target is missing/stale or another user terminal is requested, then select its returned `sessionId`; if no live PTY exists, use `terminal.create` and operate its returned sessionId without opening a Tab (`shell` is local-only; SSH uses the saved host login shell; `agentSessionId` is correlation metadata only); do not ask the user to reopen an available terminal or create a new binding.
- If a selected member is actually disconnected, call `terminal.reconnect` with its `paneId`, wait for acknowledgement, and refresh the list. If no live PTY exists, use `terminal.create` and operate its returned sessionId without opening a Tab (`shell` is local-only; SSH uses the saved host login shell; `agentSessionId` is correlation metadata only). Use `ssh.command` or `ssh.command_on_resolved_host` only when a suitable PTY cannot be created or the user explicitly requests a background structured result; their stdout/stderr do not appear in the left terminal.
...
- For SFTP copies, call `sftp.transfer.enqueue` only to receive a transfer id, track it through `sftp.transfer.list`, and use `sftp.transfer.cancel` only to stop an explicitly requested task. The 60-second MCP timeout protects those short calls only, never the queued transfer. `idleTimeoutSeconds` protects consecutive missing byte progress rather than total duration; after `failureKind = "idleTimeout"`, re-enqueue the same endpoints and adopted timeout to resume retained partial data. Do not call retired synchronous `sftp.upload*` or `sftp.download*` tools.
- MCP host policy owns confirmation, approval, permissions, hooks, and audit and may choose when to request them; Kerminal does not add a second per-command prompt or pending/confirm queue.
- Every external Agent session uses global scope across all Kerminal tabs. Prefer `targetBinding` as the current terminal; it is a convenience target, not an access restriction.
- In a session workspace, read `context/mcp-endpoint.json`, `context/target-binding.json`, and `context/terminal-snapshot.json`. For ordinary commands, inspect the current `targetBinding` with `terminal.snapshot`, then use `terminal.write` through the visible PTY so input and output remain visible in the user's left PTY.
- When targetBinding is missing/stale or another global user terminal is requested, call `kerminal.agent.target_context` and `terminal.list`, then pass the selected sessionId to `terminal.snapshot`/`terminal.write`; do not ask the user to reopen an available terminal or create a new binding.
- If no live PTY exists, use `terminal.create` with `target = "local"` or `target = "ssh"` plus a saved `hostId` when needed, then use `terminal.snapshot`/`terminal.write`/`terminal.close` on its returned `sessionId` (`shell` is local-only; SSH uses the saved host login shell; `agentSessionId` is correlation metadata only). Use `ssh.command` or `ssh.command_on_resolved_host` only when a suitable PTY cannot be created or the user explicitly requests a background structured result; their stdout/stderr do not appear in the left terminal.
