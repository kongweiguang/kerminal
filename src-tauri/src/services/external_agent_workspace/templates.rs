...
| `hosts/*.toml` | Host metadata for SSH/SFTP/Telnet/Serial/RDP/container targets | Filename must be `<id>.toml`; SFTP-only hosts use schema v2 with `protocol = "sftp"`; saved secret values are referenced by `secret_ref` / `key_passphrase_ref` and encrypted in the vault. |
...
Legacy SSH/RDP/Telnet/Serial hosts remain schema v1. Kerminal infers their protocol from the existing tags and does not migrate them in bulk:
...
An independent SFTP-only host is deliberately schema v2 so older Kerminal versions fail closed instead of opening it as an SSH terminal:
...
- `protocol`: omitted for schema v1; schema v2 currently accepts only `sftp` and requires it.
...
- Use schema v2 only for independent SFTP-only hosts and always pair it with `protocol = "sftp"`. Keep all other hosts on schema v1; do not add `protocol` to schema v1.
...
3. For an SFTP-only host write `schema_version = 2` and `protocol = "sftp"`; otherwise retain schema v1 without a protocol field. Fill every required host field, including `production = true` or `production = false`.
...
- SFTP host is rejected: confirm `schema_version = 2` and `protocol = "sftp"`; schema v2 with a missing or different protocol intentionally fails closed.
