...
pub fn ensure_current_schema(conn: &mut Connection) -> AppResult<()> {
...
fn initialize_current_schema(conn: &mut Connection) -> AppResult<()> {
    let tx = conn.transaction()?;
    tx.execute_batch(COMMAND_SCHEMA)?;
    tx.execute_batch(SNIPPET_PREFERENCES_SCHEMA)?;
...
CREATE TABLE IF NOT EXISTS command_suggestion_provider_cache (
    provider           TEXT NOT NULL
        CHECK (provider IN ('history', 'remotePath', 'remoteCommand', 'git', 'spec', 'snippet')),
...
const SNIPPET_PREFERENCES_SCHEMA: &str = r#"
